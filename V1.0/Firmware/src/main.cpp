...
void setup()
{
    ...
    // System OFF wake and a normal cold boot both begin with the current GPIO
    // level. Queue only actual mouse buttons; the separate multi-host button
    // must never become a mouse click.
    startupWakeButtons = buttonsCurrent();
