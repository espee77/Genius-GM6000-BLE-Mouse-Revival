...
static int8_t smoothScrollFromRawY(long rawY) {
    ...
    scrollVelocity *= SCROLL_MOMENTUM_DAMPING;

    // Kleine restjes stoppen
    if (rawY == 0 && scrollVelocity > -SCROLL_STOP_VELOCITY_THRESHOLD && scrollVelocity < SCROLL_STOP_VELOCITY_THRESHOLD) {
        scrollVelocity = 0.0f;
    }
...
bool ballMouseHasPendingWork(uint8_t buttons) {
    if (encodersHasPendingMovement()) {
        return true;
    }

    // Scroll momentum must continue to be serviced even after the ball stops.
    if ((buttons & MOUSE_BUTTON_MIDDLE) != 0) {
        return true;
    }

    return scrollVelocity <= -SCROLL_STOP_VELOCITY_THRESHOLD ||
           scrollVelocity >=  SCROLL_STOP_VELOCITY_THRESHOLD ||
           scrollAccumulator <= -SCROLL_STOP_ACCUMULATOR_THRESHOLD ||
           scrollAccumulator >=  SCROLL_STOP_ACCUMULATOR_THRESHOLD;
}
...
MouseReportData ballMouseUpdate(uint8_t buttons, bool &activity) {
    ...
    if (middleScrollMode) {
        ...
        scrollVelocity *= SCROLL_EXIT_DAMPING;
        scrollAccumulator *= SCROLL_EXIT_DAMPING;

        if (scrollVelocity > -SCROLL_STOP_VELOCITY_THRESHOLD && scrollVelocity < SCROLL_STOP_VELOCITY_THRESHOLD) {
            scrollVelocity = 0.0f;
        }

        if (scrollAccumulator > -SCROLL_STOP_ACCUMULATOR_THRESHOLD && scrollAccumulator < SCROLL_STOP_ACCUMULATOR_THRESHOLD) {
